...
@section('content')
    <div class="page-header"><h3 class="fw-bold mb-3">KYC Review Queue</h3></div>
                                    ...
                                    @if ($doc->status === 'pending')
                                        <div class="d-flex flex-column gap-1" style="min-width: 100px;">
                                            <form method="POST" action="{{ route('affiliates.kyc.review', $doc) }}">
                                                @csrf @method('PUT')
                                                <button name="status" value="approved" class="btn btn-success btn-sm w-100" onclick="return confirm('Approve this document?')">Approve</button>
                                            </form>
                                            <form method="POST" action="{{ route('affiliates.kyc.review', $doc) }}">
                                                @csrf @method('PUT')
                                                <button name="status" value="rejected" class="btn btn-danger btn-sm w-100" onclick="return confirm('Reject this document?')">Reject</button>
                                            </form>
                                        </div>
                                    @else
